...
class DilatedNetwork(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_hidden_features,n_resblocks, dropout=0, padding=1, kernel_size=3,batch_norm=True):
        ...
        dilatations = [2**(k+1) for k in range(len(num_hidden_features))]
        ...
        for features_in,features_out in [num_features[i:i+2] for i in range(0,len(num_features), 1)][:-1]:
            ...
            for _ in range(n_resblocks):
                ...
                block += [ResidualBlock(features_out, kernel_size, padding, dropout=dropout,batch_norm=batch_norm)]
            ...
            step+=1
        block +=[nn.Conv2d(num_hidden_features[-1], out_channels, kernel_size=kernel_size,stride=1, padding=padding)]
...
#     Sequential Dialted convolutions
#     """
#     def __init__(self, n_channels, n_convolutions,dropout,kernel_size,blockObject=ResidualBlock,batch_norm=True):
#         super(DilatedConvolutions2, self).__init__()
#         self.dilatations = [2**(k+1) for k in range(n_convolutions)]
#         self.blocks = nn.ModuleList([blockObject(n_channels, kernel_size, d, dropout=dropout, dilation=d,batch_norm=batch_norm) for d in self.dilatations ])
#     def forward(self,x):
