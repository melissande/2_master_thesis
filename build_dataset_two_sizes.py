...
if __name__ == '__main__':
    ...
    #path_patches='../2_DATA_GHANA/RAW_PATCHES/256_x_256/'
    ...
    path_dataset_all=[]
    path_dataset=Path_dataset+'256_x_256_8_pansh/'
    if not os.path.exists(path_dataset):
            os.makedirs(path_dataset)
    index=np.arange(9)
...
#     path_dataset_all.append([path_dataset,index])
    ...
    for i in range(training_size):
        ...
        for path in path_dataset_all:
            input_all=np.transpose(list_input[i,:,:,path[1]],(1,2,0))
            output_all=list_output[i]
            if path[0]==Path_dataset+'256_x_256_8_pansh/':
                write_data_h5(path[0]+'TRAINING/INPUT/input_'+str(i)+'.h5',input_all)
                write_data_h5(path[0]+'TRAINING/OUTPUT/output_'+str(i)+'.h5',output_all)
            if path[0]==Path_dataset+'128_x_128_8_pansh/':
                input_all_128=[]
                output_all_128=[]
                if (np.sum(input_all[:128,:128,:])).astype(int)!=0:
                    input_all_128.append(input_all[:128,:128,:])
                    output_all_128.append(output_all[:128,:128])
                if (np.sum(input_all[128:,:128,:])).astype(int)!=0:
                    input_all_128.append(input_all[128:,:128,:])
                    output_all_128.append(output_all[128:,:128])
                if (np.sum(input_all[:128,128:,:])).astype(int)!=0:
                ...
                if (np.sum(input_all[128:,128:,:])).astype(int)!=0:
                    input_all_128.append(input_all[128:,128:,:])
                    output_all_128.append(output_all[128:,128:])
                input_all_128=np.asarray(input_all_128)
                output_all_128=np.asarray(output_all_128)
                for j in range(len(input_all_128)):
                    write_data_h5(path[0]+'TRAINING/INPUT/input_'+str(len(input_all_128)*i+j)+'.h5',input_all_128[j])
                    write_data_h5(path[0]+'TRAINING/OUTPUT/output_'+str(len(output_all_128)*i+j)+'.h5',output_all_128[j])
    ...
    for i in range(training_size,training_size+validation_size):
        ...
        for path in path_dataset_all:
            ...
            output_all=list_output[i]
            if path[0]==Path_dataset+'256_x_256_8_pansh/':
                write_data_h5(path[0]+'VALIDATION/INPUT/input_'+str(i)+'.h5',input_all)
                write_data_h5(path[0]+'VALIDATION/OUTPUT/output_'+str(i)+'.h5',output_all)
            if path[0]==Path_dataset+'128_x_128_8_pansh/':
                input_all_128=[]
                output_all_128=[]
                if (np.sum(input_all[:128,:128,:])).astype(int)!=0:
                ...
                output_all_128=np.asarray(output_all_128)
                for j in range(len(input_all_128)):
                    write_data_h5(path[0]+'VALIDATION/INPUT/input_'+str(len(input_all_128)*i+j)+'.h5',input_all_128[j])
                    write_data_h5(path[0]+'VALIDATION/OUTPUT/output_'+str(len(output_all_128)*i+j)+'.h5',output_all_128[j])
    ...
    for i in range(training_size+validation_size,list_input.shape[0]):
        ...
        for path in path_dataset_all:
            ...
            output_all=list_output[i]
            if path[0]==Path_dataset+'256_x_256_8_pansh/':
                write_data_h5(path[0]+'TEST/INPUT/input_'+str(i)+'.h5',input_all)
                write_data_h5(path[0]+'TEST/OUTPUT/output_'+str(i)+'.h5',output_all)
            if path[0]==Path_dataset+'128_x_128_8_pansh/':
                input_all_128=[]
                output_all_128=[]
                if (np.sum(input_all[:128,:128,:])).astype(int)!=0:
                ...
                output_all_128=np.asarray(output_all_128)
                for j in range(len(input_all_128)):
                    write_data_h5(path[0]+'TEST/INPUT/input_'+str(len(input_all_128)*i+j)+'.h5',input_all_128[j])
                    write_data_h5(path[0]+'TEST/OUTPUT/output_'+str(len(output_all_128)*i+j)+'.h5',output_all_128[j])
